...
const router = Router();
import { createTransport } from 'nodemailer'; //메일 전송을 위한 모듈
import { createHash, randomBytes } from 'crypto'; //비밀번호 인증키 역할을 할 토큰 생성을 위한 모듈 
...
import { user as __user, pass as _pass } from '../db-config/emailsend-config.json';
...
router.post('/join', function (req, res, next) {
    ...
    tz.setDefault("Asia/Seoul"); //한국시간으로 설정
    ...
    const hashedPwd = createHash("sha512").update(ownerPwd + salt).digest("hex");
...
router.post('/login', function (req, res) {
    ...
    connection.query(sql, ownerEmail, function (err, result) {
        ...
        if (err) {
            ...
            } else if ((createHash("sha512").update(ownerPwd + result[0].OwnerSalt).digest("hex"))
